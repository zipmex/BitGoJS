...
describe('Wallet Prototype Methods', function() {
  ...
  describe('Accelerate Transaction (server mocked)', function() {
    ...
    let parentTxId = '75cfc5a7b214c4b73c92c7b02608cde70b226767a9576f84c04407e43fd385bd';
    const outputIdx = 0;
    ...
    describe('bad input', function() {
      ...
      it('no outputs back to own wallet', co(function *() {
        ...
        try {
          ...
          e.message.should.include(`No outputs to wallet in parent transaction`);
        }
      }));
    ...
    describe('successful tx acceleration', function() {
      ...
      it('accelerates a stuck tx without additional unspents', co(function *() {
        ...
        .reply(200, {
          unspents: [
            {
              tx_hash: '75cfc5a7b214c4b73c92c7b02608cde70b226767a9576f84c04407e43fd385bd',
              tx_output_n: 0,
              value: 10348500,
      ...
      it('accelerates a stuck tx with one additional segwit unspent', co(function *() {
        ...
        .reply(200, {
          unspents: [
            {
              tx_hash: '8815f202c8654b6c8b295749545c711878cd845a14cb1ea982394d0c14945c33',
              tx_output_n: 0,
              value: 10000,
      ...
      it('accelerates a stuck tx with one additional P2SH unspent', co(function *() {
        parentTxId = '8815f202c8654b6c8b295749545c711878cd845a14cb1ea982394d0c14945c33';
        nock(bgUrl)
        .get(`/api/v1/wallet/${wallet.id()}/tx/${parentTxId}`)
        .reply(200, {
  ...
  describe('Accelerate Transaction (test server)', function() {
    ...
    const verifyTargetFeeRate = function({ parentTx, childTx, targetRate }, callback) {
      return co(function *() {
        ...
        const childSize = child.body.transaction.vsize;
        const parentFee = parent.body.transaction.fee_int;
        const parentSize = parent.body.transaction.vsize;

        const combinedSize = childSize + parentSize;
        const combinedFee = childFee + parentFee;
        const combinedRate = combinedFee / combinedSize;

        combinedRate.should.approximately(targetRate, 1);
    ...
    describe('successful tx acceleration', function() {
      ...
      it('accelerates a stuck tx', co(function *() {
        ...
        yield verifyTargetFeeRate({ parentTx, childTx, targetRate: feeRate });
      }));
    });
  });
});
